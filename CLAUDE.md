...
**Client (`client/src/`).** Heavy use of React Context — one provider per concern under `context/` (`SpriteContext`, `CanvasContext`, `PaletteSelectedContext`, `OpenAISettingsContext`, `GenerationMethodContext`, `AssetTypeContext`, etc.), all composed in `providers/AppProviders.tsx`. Feature folders under `features/`: `InputSection` (prompt/image input + generation), `ExportSection` (the four export formats), and `SpriteEditor` (a full pixel-art editor with its own components/contexts/hooks/libs/Sidebar). Network calls go through `api/` (`generateImageApi.ts`, `moderatePrompt.ts`) against the base URL from `VITE_API_URL`. Profanity is pre-filtered client-side with `bad-words` before hitting the moderation endpoint; code export is syntax-highlighted with `prismjs`.
