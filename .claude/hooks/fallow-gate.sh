...
cleanup() {
  ...
  # `fallow audit` materializes the base ref as a temp git worktree (a perf
  # cache) that otherwise lingers in `git worktree list`. Remove any
  # fallow-audit-base-cache worktrees this repo registered. They are pure cache;
  # fallow recreates them on the next audit, so this trades a little speed for a
  # tidy worktree list.
  if command -v git >/dev/null 2>&1; then
    { git worktree list --porcelain 2>/dev/null \
        | sed -n 's#^worktree \(.*fallow-audit-base-cache-.*\)$#\1#p' \
