...
| `pixellab-generate`  | `POST /generate-image/pixellab` | ⚠️ 402 Payment Required (see below) |
...
- **Image** responses are non-deterministic; the oracle is the **shape**: a
  ...
  `get_final_size` (square branch). `samples/openai-dragon.png` is a decoded sample.

## Notable findings

1. **PixelLab returned `402 Payment Required`** from `POST https://api.pixellab.ai/v1/generate-image-pixflux`.
   This is an account-billing state (no credits), **not** a code bug — it confirms
   the endpoint + Bearer auth are correct. A success body could not be recorded
   until the PixelLab account is funded. The Bun rewrite targets the same endpoint.
2. **Moderation `categories`/`category_scores` contain BOTH key formats** — snake_case
